...
# Dataset paths
train_dir = config.HIST_TRAIN  # Update via MLAG_DATA_ROOT env var or config.py
val_dir = config.HIST_VAL  # Update via MLAG_DATA_ROOT env var or config.py
...
log_file_path = os.path.join(log_dir, 'training_hist.txt')
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, patience=10):
    ...
    if best_model_wts is not None:
        ...
        torch.save(best_model_wts, os.path.join(model_dir, 'his_best_model.pth'))

img_save_dir = config.IMG_DIR_HIS
...
evaluate_and_plot(
    model_path=os.path.join(model_dir, 'his_best_model.pth'),
