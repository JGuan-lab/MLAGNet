...
# # Dataset paths
# train_dir = r'/home/yons/data/Weiyubing/fenleidata/mhsit/train'  # Update this path to your training folder
# val_dir = r'/home/yons/data/Weiyubing/fenleidata/mhsit/val'  # Update this path to your validation folder
...
# model_dir = r"/home/Yb/uureal/identity/model/node"
# os.makedirs(model_dir, exist_ok=True)

# # Define a path for the training log file
# log_file_path = os.path.join(log_dir, 'training_logsmhisit.txt')
...
# Dataset paths
train_dir = config.MHIST_TRAIN  # Update via MLAG_DATA_ROOT env var or config.py
val_dir = config.MHIST_VAL  # Update via MLAG_DATA_ROOT env var or config.py
...
def train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs, patience=10):
    ...
    for epoch in range(num_epochs):
        ...
        # --- save model every 10 epochs to avoid losing progress if a late error occurs ---
        if (epoch + 1) % 10 == 0:
            periodic_save_path = os.path.join(model_dir, f'model_mhist_epoch_{epoch + 1}.pth')
            torch.save(model.state_dict(), periodic_save_path)
            print(f'Periodic checkpoint saved at epoch {epoch + 1}: {periodic_save_path}')
    ...
    if best_model_wts is not None:
        print(f'Saving best model with Val Loss: {best_val_loss:.4f}')
        final_save_path = os.path.join(model_dir, 'best_modelmhist.pth')
        try:
            torch.save(best_model_wts, final_save_path)
            print(f"Best model saved successfully to: {final_save_path}")
        except Exception as e:
            print(f"Final save failed: {e}. Please check directory permissions.")
img_save_dir = config.IMG_DIR_MHIST
...
train_model(model, train_loader, val_loader, criterion, optimizer, num_epochs=50)
evaluate_and_plot(
    model_path=os.path.join(model_dir, 'best_modelmhist.pth'),
