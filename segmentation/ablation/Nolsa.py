...
def train_model(model, train_loader, val_loader, device, cfg):
    ...
    with open(os.path.join(cfg['TRAIN']['log_dir'], 'train_log_retinaabi02.txt'), 'w') as log_file:
...
cfg = {
    'TRAIN': {
        ...
        'epochs': 500,
        'model_dir': config.XIAORONG_MODEL_NOLSA,
        'log_dir': config.XIAORONG_LOG_NOLSA,
        'result_dir': config.XIAORONG_RESULT_NOLSA,
