...
def train_model(model, train_loader, val_loader, device, cfg):
    ...
    with open(os.path.join(cfg['TRAIN']['log_dir'], 'train_log_ani01.txt'), 'w') as log_file:
        ...
        for epoch in range(cfg['TRAIN']['epochs']):
            ...
            val_dices.append(val_dice)  # Record val Dice coefficient

            if val_accuracy > best_val_accuracy:
                best_val_accuracy = val_accuracy
                model_save_path = os.path.join(cfg['TRAIN']['model_dir'], f'model40_bestacc_{best_val_accuracy}.pth')
            ...
            if epoch == cfg['TRAIN']['epochs'] - 1:
                ...
                with open(os.path.join(cfg['TRAIN']['log_dir'], 'ani_01_val_log_ani.txt'), 'a') as val_log_file:
...
def validate_and_save_result(model, val_loader, device, cfg, epoch):
    ...
    # Write validation results to log file
    with open(os.path.join(cfg['TRAIN']['log_dir'], 'ani_01_val_log.txt'), 'a') as val_log_file:
