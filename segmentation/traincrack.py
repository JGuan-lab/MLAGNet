    ...
    with open(os.path.join(cfg['TRAIN']['log_dir'], 'train_log_ani02.txt'), 'w') as log_file:
        ...
        for epoch in range(cfg['TRAIN']['epochs']):
            ...
            if epoch == cfg['TRAIN']['epochs'] - 1:
                ...
                with open(os.path.join(cfg['TRAIN']['log_dir'], 'crack_02_val_log.txt'), 'a') as val_log_file:
...
cfg = {
    'TRAIN': {
        ...
        'epochs': 500,
        'model_dir': config.CRACK_MODEL_DIR,
        'log_dir': config.CRACK_LOG_DIR,
...
train_dataset = MedicalImageDataset(img_dir=config.CRACK_TRAIN_IMG, mask_dir=config.CRACK_TRAIN_LABEL, img_size=512)
train_loader = DataLoader(train_dataset, batch_size=1, shuffle=True)

val_dataset = MedicalImageDataset(img_dir=config.CRACK_TEST_IMG, mask_dir=config.CRACK_TEST_LABEL, img_size=512)
