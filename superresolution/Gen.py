...
class DConv(nn.Module):  # depthwise separable conv
...
class MDTA(nn.Module):
    ...
    def forward(self, x):
        ...
        #x = self.ln(x)  # use the input tensor's dimensions to define LayerNorm normalization shape
...
class GDFN(nn.Module):
    ...
    def forward(self,x):                  
        ...
        #x = nn.LayerNorm(x.size()[1:])(x)  # use the input tensor's dimensions to define LayerNorm shape
        x = F.layer_norm(x,[x.size(1), x.size(2), x.size(3)])
        #x = nn.LayerNorm([x.size(1), x.size(2), x.size(3)])(x)
        # upper branch
        up=self.upconv1(x)
...
class Up(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        x1 =self.up(x1)
        x1= self.conv(x1)
        #print("size after upsampling bottom feature",x1.shape)
        # input is CHW
...
class CBAMLayer(nn.Module):  # attention mechanism
...
class Generator(nn.Module):
    ...
    def __init__(self,in_channel,filters,num_upsample,upblocks=[1,1,1,1],downblocks=[1,1,1,1]):
    ...
    def post_process(self,x):
        _,_,h,w=x.shape
        scale_factor = 2 ** self.num_upsample  # total upsample factor
        #scale_factor = 4
        x = x[:, :, 0:h - self.mod_pad_h*scale_factor , 0:w - self.mod_pad_w*scale_factor]
        return x
...
# create random input images (for demonstration only)
...
flops,params = profile(u_model,inputs = (dummy_images,))
print(f"==========================")
