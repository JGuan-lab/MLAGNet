...
from basicsr.utils import file_client
...
class RealESRGANDatadegration():
    def __init__(self,img_paths):
        ...
        self.scale = 4  # scale factor
        #self.gt_size = gt_size  # input image size, requires h == w
        gpu_id = None  # 0
        device = None  #
        if gpu_id:
        ...
        self.resize_prob = [0.2, 0.6, 0.2]  # up, down, keep
        ...
        self.kernel_prob =[0.55, 0.25, 0.12, 0.03, 0.02, 0.03]  # a list for each kernel probability
        self.sinc_prob = 0.1
        self.blur_sigma = [0.2, 2]  # standard deviation of blur kernel
        ...
        self.kernel_prob2 = [0.55, 0.25, 0.12, 0.03, 0.02, 0.03]
        self.sinc_prob2 = 0.1
        self.blur_sigma2 = [0.2, 1]
...
if __name__ == '__main__':
    filepath = config.DEBUG_DEGRAD_IN
    filepath2 = config.DEBUG_DEGRAD_OUT
    l_dir = os.listdir(filepath)
    for file in l_dir:
        full_path = os.path.join('%s\%s' % (filepath, file))
        t = RealESRGANDatadegration(full_path)
        result = t.synthesis()
        full_path2 = os.path.join('%s\%s' % (filepath2, file))
        torchvision.utils.save_image(result, full_path2)