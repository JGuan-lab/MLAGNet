    ...
    with open(results_file_path, 'w') as results_file, open(error_file_path, 'w') as error_file:
        ...
        for model_filename in os.listdir(model_folder_path):
            if model_filename.endswith(".pth") and (model_filename.startswith("netG") or model_filename.startswith("best")):
                ...
                sr_result_folder_path = os.path.join(config.SR_TMP_RESULTS_8, model_filename.split('.')[0])  # temporary result folder
...
def main():
    model_folder_path = config.SR_TEST_MODEL_8FULL  # model folder
    lr_folder_path = config.SR_KAGGLE_8X  # LR image folder
    hr_folder_path = config.SR_KAGGLE_TOTAL  # HR image folder
    results_file_path = config.SR_RESULT_TXT_8COVID  # results file
